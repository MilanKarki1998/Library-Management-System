...
if($request==1){
	...
	$query ="SELECT students.StudentId,students.name,books.Book_Name,books.ISBN,Issuedbooks.IssuesDate,Issuedbooks.ExpectedReturnDate,Issuedbooks.ReturnStatus,Issuedbooks.fine,Issuedbooks.ReturnDate,Issuedbooks.id as rid from  Issuedbooks inner join students on students.StudentId=Issuedbooks.StudentId inner join books on books.id=Issuedbooks.BookId where Issuedbooks.ReturnStatus=1 ";
	$result = mysqli_query($conn,$query);
}elseif($request==3){
	$query ="SELECT students.StudentId,students.name,books.Book_Name,books.ISBN,Issuedbooks.IssuesDate,Issuedbooks.ExpectedReturnDate,Issuedbooks.ReturnStatus,Issuedbooks.fine,Issuedbooks.ReturnDate,Issuedbooks.id as rid from  Issuedbooks inner join students on students.StudentId=Issuedbooks.StudentId inner join books on books.id=Issuedbooks.BookId where Issuedbooks.ReturnStatus is NULL ";
	$result = mysqli_query($conn,$query);
}
				$cnt=1;
				$a=0;
