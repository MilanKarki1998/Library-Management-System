...
if($request==1){
	...
	$query ="SELECT students.StudentId,students.name,books.Book_Name,books.ISBN,Issuedbooks.IssuesDate,Issuedbooks.ExpectedReturnDate,Issuedbooks.ReturnStatus,Issuedbooks.fine,Issuedbooks.ReturnDate,Issuedbooks.id as rid from  Issuedbooks inner join students on students.StudentId=Issuedbooks.StudentId inner join books on books.id=Issuedbooks.BookId where Issuedbooks.ReturnDate >= CURRENT_DATE() order by Issuedbooks.id desc";
	$result=mysqli_query($conn,$query);
}elseif($request==3){
	$query ="SELECT students.StudentId,students.name,books.Book_Name,books.ISBN,Issuedbooks.IssuesDate,Issuedbooks.ExpectedReturnDate,Issuedbooks.ReturnStatus,Issuedbooks.fine,Issuedbooks.ReturnDate,Issuedbooks.id as rid from  Issuedbooks inner join students on students.StudentId=Issuedbooks.StudentId inner join books on books.id=Issuedbooks.BookId where Issuedbooks.ReturnDate > DATE_SUB(DATE(NOW()), INTERVAL 1 WEEK) order by Issuedbooks.id desc";
	$result=mysqli_query($conn,$query);
}elseif($request==4){
	...
	$result=mysqli_query($conn,$query);
				...
				if(mysqli_num_rows($result)>0){
					...
					echo "<h1>No Books are Returned<h1>";
				?>
				</table>
<?php }		
?>